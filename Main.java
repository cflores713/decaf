...
public class Main extends Application implements  EventHandler<ActionEvent>{
	...
	public void start(Stage s) {
		  ...
		  //----------------- Execute tab-------------------//
		    ...
		    // create a VBox 
		    StringBinding bind = Bindings.createStringBinding(() -> {//this function creates between the user typing and the number of characters being typed
                ...
                return "Number of characters: " + charNum + "\n"
				+ "Number of lines: " + lineNum + "\n"
				+ "Keywords in use (if, else, while, for): " + keyNum + "\n";
            }, text.textProperty());
		    label.textProperty().bind(bind);//updates the word count everytime user types
		   
		    VBox vb = new VBox(mb); 
		    text.setPrefHeight(1000); 
		    text.setPrefWidth(1000); 
		    vb.getChildren().add(text);
		    vb.getChildren().add(bPane);//adds the panel to the frame of the gui
		    bPane.setBottom(label); //sets the word count panel at the bottom
		    ...
		    s.setOnCloseRequest( ev -> {//when user clicks the exit window button a prompt will show if they would like to save what they have
		     exitOptions();//Need to fix the cancel button
		    });
...
//                path = file.getPath();//gets the path of the file
	 ...
	 public TreeItem<String> getNodesForDirectory(File directory) { //Returns a TreeItem representation of the specified directory
	        ...
	        for(File c: directory.listFiles()) {
	            ...
	            if(c.isDirectory()) { //Then we call the function recursively
	                root.getChildren().add(getNodesForDirectory(c));
	            } else {
	                root.getChildren().add(new TreeItem<String>(c.getName()));
	            }
	        }
	        return root;
	 ...
	 void CheckCount()//Converted the word counter to a function so that it can be reuseable
	 {
			//ToDo: Figure out where to have this result show up inside the application, not just in console
			int charNum = 0;
			int lineNum = 0;
...
//			compilationTask.call();
		...
		{
		CheckCount();//calls the word count function
		}
    }
